...
@export_range(0.0, 360.0) var node_angle := 0.0:
	set(val):
		node_angle = val
		rotation = fposmod(deg_to_rad(val), TAU)
@export_range(-3.0, 3.0) var spin_speed : float
...
func _ready() -> void:
	...
	rotation += deg_to_rad(node_angle)
...
func _on_health_component_died() -> void:
	...
	if particles_node and explosion:
		...
		die_explosion.rotation = rotation
